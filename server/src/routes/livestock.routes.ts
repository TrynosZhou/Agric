...
import {
  individualSpecies,
  LivestockAnimal,
  type IndividualSpecies
} from "../entities/LivestockAnimal";
import { PoultryBatch } from "../entities/PoultryBatch";

const router = Router();
const animalRepo = () => AppDataSource.getRepository(LivestockAnimal);
const poultryRepo = () => AppDataSource.getRepository(PoultryBatch);
...
router.get("/", async (req, res) => {
  ...
  const where: { species?: IndividualSpecies } = {};
  if (species && individualSpecies.includes(species as IndividualSpecies)) {
    where.species = species as IndividualSpecies;
  }
...
router.post("/animals", async (req, res) => {
  const species = String(req.body?.species ?? "").trim().toLowerCase() as IndividualSpecies;
  const tagIdNumber = String(req.body?.tagIdNumber ?? "").trim();
  const dateOfBirth = String(req.body?.dateOfBirth ?? "").trim();
  if (!individualSpecies.includes(species)) {
...
router.post("/animals/:id/health-events", async (req, res) => {
  ...
  if (eventType === "weight") {
    ...
    res.status(400).json({
      message:
        "eventType must be one of: weight, vaccination, dipping, treatment, milk-production, mortality"
    });
    return;
  }
...
router.put("/poultry-batches/:id", async (req, res) => {
  ...
  if (req.body.flockSize !== undefined) batch.flockSize = Number(req.body.flockSize);
  if (req.body.mortalityCount !== undefined) batch.mortalityCount = Number(req.body.mortalityCount);
  if (req.body.feedConsumptionKg !== undefined) batch.feedConsumptionKg = String(req.body.feedConsumptionKg);
  if (req.body.eggProduction !== undefined) batch.eggProduction = Number(req.body.eggProduction);
  await poultryRepo().save(batch);
  res.json(batch);
});
