...
import { Asset, assetCategories, assetConditions, type AssetCategory, type AssetCondition } from "../entities/Asset";
import {
  IrrigationSystem,
  irrigationStatuses,
  irrigationTypes,
  ...
  type IrrigationType,
  type PowerSource
} from "../entities/IrrigationSystem";

const router = Router();
const assetRepo = () => AppDataSource.getRepository(Asset);
const irrigationRepo = () => AppDataSource.getRepository(IrrigationSystem);
...
router.post("/assets", async (req, res) => {
  ...
  const category = String(req.body?.category ?? "").trim().toLowerCase() as AssetCategory;
  const quantityAvailable = Number(req.body?.quantityAvailable ?? 0);
  const quantityInUse = Number(req.body?.quantityInUse ?? 0);
  const condition = String(req.body?.condition ?? "good").trim().toLowerCase() as AssetCondition;
  ...
  if (!assetConditions.includes(condition)) {
...
router.put("/assets/:id", async (req, res) => {
  ...
  if (req.body.condition !== undefined) {
    const condition = String(req.body.condition).trim().toLowerCase() as AssetCondition;
    if (!assetConditions.includes(condition)) {
  ...
  if (req.body.quantityAvailable !== undefined) asset.quantityAvailable = Number(req.body.quantityAvailable);
  if (req.body.quantityInUse !== undefined) asset.quantityInUse = Number(req.body.quantityInUse);
...
router.post("/irrigation-systems", async (req, res) => {
  const systemType = String(req.body?.systemType ?? "").trim().toLowerCase() as IrrigationType;
  const coverageAreaOrCapacity = String(req.body?.coverageAreaOrCapacity ?? "").trim();
  const powerSource = String(req.body?.powerSource ?? "").trim().toLowerCase() as PowerSource;
  const status = String(req.body?.status ?? "inactive").trim().toLowerCase() as IrrigationStatus;
  if (!irrigationTypes.includes(systemType)) {
    res.status(400).json({ message: "systemType must be center pivot or pump" });
    return;
  }
  if (!powerSources.includes(powerSource)) {
    res.status(400).json({ message: "powerSource must be diesel, electric or solar" });
    return;
  }
  if (!irrigationStatuses.includes(status)) {
...
router.put("/irrigation-systems/:id", async (req, res) => {
  ...
  if (req.body.systemType !== undefined) {
    const type = String(req.body.systemType).trim().toLowerCase() as IrrigationType;
    if (!irrigationTypes.includes(type)) {
  ...
  if (req.body.powerSource !== undefined) {
    const source = String(req.body.powerSource).trim().toLowerCase() as PowerSource;
  ...
  if (req.body.status !== undefined) {
    const status = String(req.body.status).trim().toLowerCase() as IrrigationStatus;
    if (!irrigationStatuses.includes(status)) {
...
router.post("/irrigation-systems/:id/fuel-usage", async (req, res) => {
  ...
  const unit = String(req.body?.unit ?? "").trim().toLowerCase() as "liters" | "kwh";
...
router.get("/maintenance-schedule", async (_req, res) => {
  ...
  const assetEntries = assets.flatMap((asset) =>
    (asset.maintenanceLogEntries ?? [])
  ...
  const irrigationEntries = irrigationSystems.flatMap((system) =>
    (system.scheduledMaintenanceDates ?? []).map((date) => ({
  ...
  const entries = [...assetEntries, ...irrigationEntries].sort((a, b) =>
