...
const workerRepo = () => data_source_1.AppDataSource.getRepository(Worker_1.Worker);
const attendanceRepo = () => data_source_1.AppDataSource.getRepository(WorkerAttendance_1.WorkerAttendance);
...
router.post("/workers", async (req, res) => {
    ...
    const casualPayMode = String(req.body?.casualPayMode ?? "day").trim().toLowerCase();
    const assignedTasks = Array.isArray(req.body?.assignedTasks)
        ? req.body.assignedTasks.map((task) => String(task))
        : [];
    if (!name) {
        res.status(400).json({ message: "name is required" });
        return;
    }
    if (!Worker_1.workerTypes.includes(type)) {
...
router.get("/attendance", async (req, res) => {
    ...
    if (from)
        qb.andWhere("attendance.attendanceDate >= :from", { from });
    if (to)
        qb.andWhere("attendance.attendanceDate <= :to", { to });
...
router.get("/payroll-summary", async (req, res) => {
    ...
    const cropBreakdown = new Map();
    const livestockBreakdown = new Map();
    for (const row of attendance) {
        const worker = row.worker;
        if (!worker)
            continue;
